...
expect_get admin/flat-client-config/list -o "$tmp_dir"/list-4.json
expect_json_eq '{"a": 123, "b": "bbb", "c": "ccc", "d": -42}' \
               "$tmp_dir"/list-4.json

# Bad JSON values should be refused.
expect_post_error 400 admin/flat-client-config/update \
            -H "Authorization: $token" \
            -H "Content-Type: application/json" \
            --data 'null'
expect_post_error 400 admin/flat-client-config/update \
...
expect_get admin/flat-client-config/list -o "$tmp_dir"/list-5.json
expect_json_eq '{"a": 123, "b": "bbb", "c": "ccc", "d": -42}' \
               "$tmp_dir"/list-5.json
