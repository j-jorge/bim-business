...
expect_post game-servers/register \
...
expect_post_error 500 game-servers/register \
                  -H "Authorization: $admin_token" \
                  -H "Content-Type: application/json" \
                  --data '{"id": "valid-server", "description": "Some server."}'
...
expect_post_error 400 game-servers/register \
                  -H "Authorization: $admin_token" \
                  -H "Content-Type: application/json" \
...
expect_get game-servers/list \
           -H "Authorization: $admin_token" \
           -o "$tmp_dir"/list-1.json
# Can't get the list if we are no admin.
expect_get_error 401 game-servers/list \
...
expect_post game-servers/keep-alive \
            -H "Content-Type: application/json" \
            --data \
            '{
               "token": "'"$gs_token_2"'",
...
expect_post_error 422 game-servers/keep-alive \
                  -H "Content-Type: application/json" \
                  --data \
                  '{
                     "token": "'"$gs_token"'",
...
# we have no news.
expect_post game-servers/set-time-to-live \
            -H "Authorization: $admin_token" \
            -H "Content-Type: application/json" \
            --data '{"delay_in_minutes": 0}'
# Can't change the delay if we are no admin.
expect_post_error 401 game-servers/set-time-to-live \
...
expect_post_error 401 game-servers/set-time-to-live \
            -H "Content-Type: application/json" \
            --data '{"delay_in_minutes": 10}'
expect_post_error 422 game-servers/set-time-to-live \
            -H "Authorization: $admin_token" \
            -H "Content-Type: application/json" \
            --data '10'
