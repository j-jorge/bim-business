...
#-------------------------------------------------------------------------------
# Check the rewards when a player leaves, in a game without bots.
...
expect_post gs/game-over \
            ...
            --data '{
                      ...
                      "outcome": ["defeated", "kicked", "kicked", "victory"]
                    }'
...
expect_true grep --quiet '^coins *| *2$' "$tmp_dir"/reward-g1-user-0.txt

expect_db 'select * from game_reward
           where game_id = '"$game_id_1"'
           and user_id = '"${user_id[1]}" \
               "$tmp_dir"/reward-g1-user-1.txt
expect_true grep --quiet '^(0 rows)$' "$tmp_dir"/reward-g1-user-1.txt
...
expect_true grep --quiet '^(0 rows)$' "$tmp_dir"/reward-g1-user-3.txt
...
expect_post gs/game-over \
            ...
            --data '{
                      ...
                      "outcome": ["victory", "defeated", "kicked", "kicked"]
                    }'
expect_db_row_exists 'select * from done_game
...
expect_db 'select * from game_reward
           where game_id = '"$game_id_2"'
           and user_id = '"${user_id[0]}" \
               "$tmp_dir"/reward-g2-user-0.txt
expect_true grep --quiet '^(0 rows)$' "$tmp_dir"/reward-g2-user-0.txt

expect_db 'select * from game_reward
           where game_id = '"$game_id_2"'
           and user_id = '"${user_id[1]}" \
               "$tmp_dir"/reward-g2-user-1.txt
expect_true grep --quiet '^(0 rows)$' "$tmp_dir"/reward-g2-user-1.txt

expect_db 'select * from game_reward
           where game_id = '"$game_id_2"'
           and user_id = '"${user_id[2]}" \
               "$tmp_dir"/reward-g2-user-2.txt
expect_true grep --quiet '^(0 rows)$' "$tmp_dir"/reward-g2-user-2.txt

expect_db 'select * from game_reward
           where game_id = '"$game_id_2"'
           and user_id = '"${user_id[3]}" \
               "$tmp_dir"/reward-g2-user-3.txt
expect_true grep --quiet '^(0 rows)$' "$tmp_dir"/reward-g2-user-3.txt

expect_db_row_count 4 'from arena_stats'
check_player_stats "${user_id[0]}" 0 0 0 1
check_player_stats "${user_id[1]}" 0 2 0 0
check_player_stats "${user_id[2]}" 1 1 0 0
check_player_stats "${user_id[3]}" 0 1 0 0
